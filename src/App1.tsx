...
const profile = {
  ...
  photo: "/profile.jpg", // Replace with your photo path, e.g. public/profile.jpg
  resume: "/Ismail_Siklodi_Resume_2026.pdf",
};
...
const projects = [
  ...
  {
    title: "Member 360 Healthcare Portal",
    type: "Dashboard Application",
    icon: <Smartphone className="h-6 w-6" />,
    description:
      "Developed interactive dashboards, real-time charts, responsive layouts, and API-connected healthcare modules using Ionic and Angular.",
    tech: ["Ionic 5", "Angular", "Charts", "API Integration"],
  },
];
...
function SectionTitle({ kicker, title }: SectionTitleProps) {
  return (
    <div className="mb-10">
      <motion.p
    ...
    <main className="min-h-screen overflow-hidden bg-[#030712] text-slate-100 selection:bg-cyan-300 selection:text-black">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/60 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          <motion.a whileHover={{ scale: 1.05 }} href="#home" className="flex items-center gap-2 font-black text-white">
      ...
      <section id="home" className="relative mx-auto grid min-h-[92vh] max-w-7xl items-center gap-12 px-5 py-20 md:grid-cols-[1.1fr_0.9fr]">
        <div>
          <motion.div
          ...
          >
            <Rocket className="h-4 w-4" /> Available for Senior / Lead Full-Stack Roles
          </motion.div>
          ...
          >
            I am {profile.name}, an 8+ years Senior Full-Stack and Mobile Engineer experienced in React Native, Ionic, Angular,
            Node.js, fintech platforms, mobile banking apps, admin dashboards, security fixes, CI/CD, and team leadership.
          </motion.p>
          ...
          >
            <a href="#projects" className="group rounded-2xl bg-cyan-300 px-6 py-3 font-bold text-slate-950 shadow-[0_0_35px_rgba(103,232,249,0.35)] transition hover:-translate-y-1 hover:bg-white">
              View Projects <ExternalLink className="ml-2 inline h-4 w-4 transition group-hover:rotate-45" />
            </a>
            <a href={profile.resume} className="rounded-2xl border border-white/15 bg-white/10 px-6 py-3 font-bold text-white backdrop-blur transition hover:-translate-y-1 hover:border-cyan-300/60">
      ...
      <section id="skills" className="relative mx-auto max-w-7xl px-5 py-20">
        <SectionTitle kicker="Technical Arsenal" title="Skills that power production-grade apps" />
      ...
      <section id="projects" className="relative mx-auto max-w-7xl px-5 py-20">
        <SectionTitle kicker="Featured Work" title="Projects with real business impact" />
          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="relative">
                ...
                <p className="mb-2 text-sm font-semibold uppercase tracking-[0.25em] text-fuchsia-300">{project.type}</p>
      ...
      <section id="experience" className="relative mx-auto max-w-7xl px-5 py-20">
        <SectionTitle kicker="Career Timeline" title="8+ years of fintech, healthcare and mobile delivery" />
          ...
          {experience.map((job, index) => (
            ...
            >
              <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                ...
                <span className="rounded-full border border-white/10 bg-slate-950/70 px-4 py-2 text-sm text-slate-300">{job.period}</span>
              </div>
      ...
      <section className="relative mx-auto max-w-7xl px-5 py-20">
        <SectionTitle kicker="Highlights" title="Achievements recruiters notice fast" />
      ...
      <section id="contact" className="relative px-5 py-24">
        ...
        >
          <h2 className="text-4xl font-black text-white md:text-6xl">Let’s build something secure and scalable.</h2>
          <p className="mx-auto mt-5 max-w-2xl text-slate-300">
            Open to senior / lead roles, remote or hybrid. Best fit: Angular, React, React Native, Ionic, Node.js, fintech platforms, and mobile app delivery.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a href={`mailto:${profile.email}`} className="rounded-2xl bg-cyan-300 px-6 py-3 font-bold text-slate-950 transition hover:-translate-y-1 hover:bg-white">
              <Mail className="mr-2 inline h-4 w-4" /> Email Me
            </a>
            <a href={profile.linkedin} target="_blank" rel="noreferrer" className="rounded-2xl border border-white/15 bg-white/10 px-6 py-3 font-bold text-white transition hover:-translate-y-1 hover:border-cyan-300">
              LinkedIn
            </a>
          </div>
          <div className="mt-8 grid gap-3 text-sm text-slate-300 md:grid-cols-3">
            <p><Phone className="mr-2 inline h-4 w-4 text-cyan-300" /> {profile.phone}</p>
            <p><Mail className="mr-2 inline h-4 w-4 text-cyan-300" /> {profile.email}</p>
            <p><MapPin className="mr-2 inline h-4 w-4 text-cyan-300" /> {profile.location}</p>
          </div>
        </motion.div>
      </section>

      <footer className="border-t border-white/10 px-5 py-8 text-center text-sm text-slate-500">
        Built with React, Tailwind CSS, Framer Motion and developer energy. © {new Date().getFullYear()} {profile.name}
      </footer>
    </main>
  );
}
